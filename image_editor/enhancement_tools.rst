...
The second page allow you to customize the images corresponding to the **Input** layers. The first layer handled by the G'MIC filter is on the top of the list.
...
The next page show the G'MIC-Qt tool to select a filter working with layers. Not all G'MIC-Qt filter support this mode. In the example below, the **Montage** filter is used to compose a mosaic of selected images.
...
Press **Select Filter** button and the final post processing page will list all tasks to complete the target images.
...
The target image can be visualized in image editor after closing the wizard dialog.
