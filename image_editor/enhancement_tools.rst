...
A settings dialog appear where you can customize the images corresponding to the **Input** layers on the first tab.
...
On the second tab of this dialog, you can customize the place where the **Output** files generated by the G'MIC filter will be store, the file format, and template file name A settings dialog appear where you can customize the images corresponding to the input layers on the first tab. Depending of the filter selected, more than one files can be generated. In this case, an automatic index is appended to the template file name. If files with same names already exists on the destination, there will be overrided.
